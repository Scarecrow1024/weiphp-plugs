...
namespace Addons\Lib1\Controller;
use Home\Controller\AddonsController;

class Lib1Controller extends AddonsController{
    public function ser(){
        $this->display();
    }
    ...
    public function xujie(){
        ...
        if(strstr($content, "该书已预约")){
            $this->error('续借失败，该书已被预约', U('/addon/Lib1/Lib1/grzx/'));
        }else{
            $this->success('续借成功', U('/addon/Lib1/Lib1/grzx/'));
        }
        //echo $post;
    ...
    public function grzx(){
        $time=date('G')*3600+date('i')*60+date('s')-150;
        $tday1=$time;
        $user=M('user');
        $openid=get_openid();
        $studentid = $user->where("openid=".'"'.$openid.'"')->getField('studentid');
